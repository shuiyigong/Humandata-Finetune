...
root1 = "/share/xurunze-local/FoundationPose/1008/"
    ...
    for bag in bags:
        if bag[0] != '@' or bag[-4:] == '.zip':
            ...
            for idx in tqdm(range(0, len(left_files), B), desc=f"Processing bag {bag}"):
                ...
                imgs0 = torch.stack(imgs0).cuda().contiguous()  # [B, C, H, W]
                imgs1 = torch.stack(imgs1).cuda().contiguous()
                ...
                with torch.cuda.amp.autocast(True):
                    if imgs0.shape[0] < torch.cuda.device_count():
                        disp = model.module(imgs0, imgs1, iters=args.valid_iters, test_mode=True) 
                    else:
                        disp = model(imgs0, imgs1, iters=args.valid_iters, test_mode=True)
      
                disp = padder.unpad(disp.float())  # [B, 1, H, W] or [B, H, W]
